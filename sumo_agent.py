...
class SumoAgent:
    ...
    def __init__(self, path_set, noise_model):
    ...
    def update_state(self):
        status_tracker = map_computor.status_calculator_noise(self.noise_model)
        self.state = State(
            queue_length=np.reshape(np.array(status_tracker[0]), newshape=(1, status_tracker[11])),
            num_of_vehicles=np.reshape(np.array(status_tracker[1]), newshape=(1, status_tracker[11])),
            waiting_time=np.reshape(np.array(status_tracker[2]), newshape=(1, status_tracker[11])),
            map_feature=np.reshape(np.array(status_tracker[3]), newshape=(1, 150, 150, 1)),
            current_noise=np.reshape(np.array(status_tracker[4]),newshape=(1, status_tracker[11])),  
            predicted_noise=np.reshape(np.array(status_tracker[8]),newshape=(1, status_tracker[11])),
            co2=np.reshape(np.array(status_tracker[5]),newshape=(1, status_tracker[11])),  
            fuel=np.reshape(np.array(status_tracker[6]),newshape=(1, status_tracker[11])),
            waiting=np.reshape(np.array(status_tracker[7]),newshape=(1, status_tracker[11])),
            ...
            if_terminal=False,lanes=status_tracker[11]
        )
