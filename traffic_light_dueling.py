...
class TrafficLightDQN:
    ...
    def __init__(self, memo, f_prefix):
        ...
        self.agent = self.DIC_AGENTS[self.para_set.MODEL_NAME](num_phases=9,
                                                               num_actions=2,
    ...
    def train(self, if_pretrain, use_average, noise_model):
        ...
        f_memory.write('time|action|current_phase|next_phase|reward|q_value|co2|fuel|noise|waiting' + "\n")
        f_memory.close()
        ...
        while current_time < total_run_cnt:
            ...
            self.agent.forget(if_pretrain=False)
            # output to std out and file
   

            memory_str = '%d|%d|%d|%d|%f' \
                         '|%s|%f|%f|%f|%f' \
                         % (current_time, action,
                            state.cur_phase[0][0],
...
#             print('time = %d\taction = %d\tcurrent_phase = %d\tnext_phase = %d\treward = %f' \
#                          '\t%s\tco2 = %f\tfuel = %f\tnoise = %f\twaiting = %f' \
#                          % (current_time, action,state.cur_phase[0][0],state.next_phase[0][0],reward, repr(q_values), sum(state.co2[0]),sum(state.fuel[0]), sum(state.current_noise[0]), statistics.mean(state.waiting_time[0])))
