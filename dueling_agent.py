...
class DuelingAgent(NetworkAgent):
    ...
    def build_network(self):
        ...
        all_flatten_feature = keras.layers.concatenate(list_all_flatten_feature, axis=1, name="all_flatten_feature")
        ...
        feauture_layer = Dense(128, activation='relu')(shared_dense)
        features = Dense(128, activation='relu')(feauture_layer)


        value_stream = Dense(128,activation="relu")(features)
        values = Dense(1,activation="sigmoid")(value_stream)

        advantage_stream = Dense(128,activation="relu")(features)
        advantages = Dense(self.num_actions,activation="linear")(advantage_stream)


        qvals = values + (advantages - tf.keras.metrics.Mean(name='mean_adv')(advantages))
    ...
    def _sample_memory(self, gamma, with_priority, memory, if_pretrain):
        ...
        if with_priority:
            ...
            for i in range(len_memory):
                ...
                total_reward = reward + gamma * next_estimated_reward
                target = self.q_network.predict(
                    self.convert_state_to_input(state),verbose=0)
                pre_target = np.copy(target)
                target[0][action] = total_reward
